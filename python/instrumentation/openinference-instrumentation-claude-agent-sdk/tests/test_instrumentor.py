...
def test_tool_error_preserves_result_content() -> None:
    """_update_tool_spans_from_messages must pass result_content to end_tool_span_with_error,
    not the hardcoded string 'Tool execution error'."""
    import openinference.instrumentation.claude_agent_sdk._wrappers as wrappers

    captured: dict[str, Any] = {}

    class _CapturingTracker(wrappers._ToolSpanTrackerBase):
        def start_tool_span(
            self, tool_name: Any, tool_input: Any, tool_use_id: Any, parent_tool_use_id: Any = None
        ) -> None:
            pass
    ...
    wrappers._update_tool_spans_from_messages(message, _CapturingTracker())

    assert captured.get("tool_use_id") == "tu_123"
    # Must be the actual result_content, not the hardcoded fallback string.
    assert captured.get("error") != "Tool execution error", (
        "_update_tool_spans_from_messages passed hardcoded string instead of result_content"
    )
    assert captured.get("error") == [{"type": "text", "text": "permission denied"}]
