...
class StrandsToOpenInferenceProcessor(SpanProcessor):
    ...
    def on_end(self, span: Span):
        ...
        try:
            ...
            if self.debug:
                logger.info(
                    f"Transformed span '{span.name}': {len(original_attrs)} -> {len(transformed_attrs)} attributes"
                )
    ...
    def _set_graph_node_attributes(self, span: Span, attrs: Dict[str, Any], result: Dict[str, Any]):
        ...
        if self.debug:
            ...
            logger.info(
                f"  Graph Node: {result.get('graph.node.id')} -> Parent: {result.get('graph.node.parent_id')}"
            )

    def _handle_chain_and_llm_span(
        self, attrs: Dict[str, Any], result: Dict[str, Any], prompt: Any, completion: Any
    ):
    ...
    def _handle_tool_span(self, attrs: Dict[str, Any], result: Dict[str, Any]):
        ...
        if tool_params := attrs.get("tool.parameters"):
            ...
            input_message = {"message.role": "assistant", "message.tool_calls": [tool_call]}
        ...
        if tool_result := attrs.get("tool.result"):
            ...
            result["llm.output_messages.0.message.content"] = self._serialize_value(
                tool_result_content
            )
        ...
        for key, value in attrs.items():
            if key.startswith("tool.") and key not in [
                "tool.name",
                "tool.id",
                "tool.parameters",
                "tool.result",
    ...
    def _handle_agent_span(self, attrs: Dict[str, Any], result: Dict[str, Any], prompt: Any):
        ...
        if prompt:
            input_message = {"message.role": "user", "message.content": str(prompt)}
            result["llm.input_messages"] = json.dumps([input_message], separators=(",", ":"))
    ...
    def _map_messages(self, messages_data: Any, result: Dict[str, Any], is_input: bool):
        ...
        if isinstance(messages_data, str):
            try:
                ...
                messages_data = [
                    {"role": "user" if is_input else "assistant", "content": messages_data}
                ]
        ...
        for idx, msg in enumerate(messages_list):
            ...
            for sub_key, sub_val in msg.items():
                clean_key = (
                    sub_key.replace("message.", "") if sub_key.startswith("message.") else sub_key
                )
    ...
    def _add_input_output_values(self, attrs: Dict[str, Any], result: Dict[str, Any]):
        ...
        if span_kind == "LLM":
            ...
            if prompt := attrs.get("gen_ai.prompt"):
                ...
                result["input.mime_type"] = (
                    "text/plain" if isinstance(prompt, str) else "application/json"
                )
            ...
            if completion := attrs.get("gen_ai.completion"):
                ...
                result["output.mime_type"] = (
                    "text/plain" if isinstance(completion, str) else "application/json"
                )
