...
from smolagents import Agent, Crew, Process, Task
from smolagents_tools import SerperDevTool  # type: ignore
...
test_vcr = vcr.VCR(
    serializer="yaml",
    cassette_library_dir="tests/openinference/instrumentation/smolagents/fixtures/",
    record_mode="never",
    match_on=["uri", "method"],
)
...
@pytest.fixture()
def setup_smolagents_instrumentation(
...
def test_oitracer(
    ...
    setup_smolagents_instrumentation: Any,
) -> None:
    in_memory_span_exporter.clear()
    assert isinstance(smolagentsInstrumentor()._tracer, OITracer)


def test_smolagents_instrumentation(
    tracer_provider: TracerProvider,
    in_memory_span_exporter: InMemorySpanExporter,
    setup_smolagents_instrumentation: Any,
) -> None:
...
def test_smolagents_instrumentation_context_attributes(
    tracer_provider: TracerProvider,
    in_memory_span_exporter: InMemorySpanExporter,
    setup_smolagents_instrumentation: Any,
    session_id: str,
