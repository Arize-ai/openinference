...
class _SyncCreateInteractionStreamWrapper(_WithTracer):
...
class _AsyncCreateInteractionStreamWrapper(_WithTracer):
    ...
    async def __call__(
        ...
        if context_api.get_value(context_api._SUPPRESS_INSTRUMENTATION_KEY):
            return await wrapped(*args, **kwargs)
        request_parameters = _parse_args(signature(wrapped), *args, **kwargs)
        span_name = "AsyncGenerateContentStream.create"
        with self._start_as_current_span(
                span_name=span_name,
                attributes=get_attributes_from_request(request_parameters),
                context_attributes=get_attributes_from_context(),
