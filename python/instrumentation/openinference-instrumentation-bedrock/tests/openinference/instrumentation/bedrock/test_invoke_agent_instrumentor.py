...
def _assert_agent_call_without_traces_span_attributes(attrs: dict[str, Any]) -> None:
    assert attrs.pop("input.value") == "What is task decomposition?"
    assert attrs.pop("llm.provider") == "aws"
    assert attrs.pop("openinference.span.kind") == "AGENT"
    assert attrs.pop("output.mime_type") == "text/plain"
    assert attrs.pop("output.value") == "Sorry, I don't have enough information to answer that."
    assert not attrs


def _assert_tool_calls_with_input_params(events: list[Any], spans: Sequence[Any]) -> None:
    ...
    llm_span = [s for s in spans if s.name == "LLM"][-1]
    ...
    action_group_span = [s for s in spans if s.name == "action_group"][-1]
    ...
    orchestration_span = [s for s in spans if s.name == "orchestrationTrace"][-1]
...
def _assert_tool_calls_without_input_params(events: list[Any], spans: Sequence[Any]) -> None:
    assert len(events) == 10
    assert len(spans) == 5
    span_names = [s.name for s in spans]
    assert span_names == [
    ...
    llm_span = [s for s in spans if s.name == "LLM"][0]
    llm_span_attributes = dict(llm_span.attributes or {})
    assert llm_span_attributes.pop("input.mime_type") == "text/plain"
...
def _assert_knowledge_base_results(events: list[Any], spans: Sequence[Any]) -> None:
    assert len(events) == 6
    span_names = [s.name for s in spans]
    assert span_names == [
        "knowledge_base",
    ...
    assert llm_span_attributes.pop("openinference.span.kind") == "LLM"
    assert not llm_span_attributes
    knowledge_base_span = [s for s in spans if s.name == "knowledge_base"][-1]
...
def _assert_preprocessing_trace(events: list[Any], spans: Sequence[Any]) -> None:
    assert len(events) == 7
    assert len(spans) == 5
    span_names = [s.name for s in spans]
    assert span_names == [
    ...
    preprocessing_span = [s for s in spans if s.name == "preProcessingTrace"][-1]
    ...
    initial_span = [s for s in spans if s.name == "bedrock_agent.invoke_agent"][-1]
...
def _assert_post_processing_trace(events: list[Any], spans: Sequence[Any]) -> None:
    assert len(events) == 7
    assert len(spans) == 5
    span_names = [s.name for s in spans]
    assert span_names == [
    ...
    postprocessing_span = [s for s in spans if s.name == "postProcessingTrace"][-1]
...
def _assert_multi_agent_collaborator(events: list[Any], spans: Sequence[Any]) -> None:
    assert len(events) == 34
    assert len(spans) == 15
    span_name_counter = Counter([s.name for s in spans])
    ...
    supervisor_agent_span = [s for s in spans if s.name == "agent_collaborator[SimpleSupervisor]"][
        -1
    ]
    ...
    math_agent_span = [s for s in spans if s.name == "agent_collaborator[MathSolverAgent]"][-1]
    ...
    orchestration_spans = [s for s in spans if s.name == "orchestrationTrace"]
...
def _assert_streaming_with_guardrails(events: list[Any], spans: Sequence[Any]) -> None:
    ...
    guardrail_spans = [s for s in spans if s.name == "Guardrails"]
    ...
    post_guardrail_span = [s for s in spans if s.name == "Guardrails"][-1]
    ...
    pre_guardrail_trace_span = [s for s in spans if s.name == "preGuardrailTrace"][0]
    ...
    post_guardrail_trace_span = [s for s in spans if s.name == "postGuardrailTrace"][0]
    ...
    orchestration_span_guardrails = [s for s in spans if s.name == "orchestrationTrace"][0]
...
def _assert_guardrail_intervention(events: list[Any], spans: Sequence[Any]) -> None:
    assert len(events) == 2
    assert len(spans) == 3
    span_names = [s.name for s in spans]
    assert span_names == [
    ...
    guardrail_span = [s for s in spans if s.name == "Guardrails"][-1]
...
def _assert_routing_classifier_trace(events: list[Any], spans: Sequence[Any]) -> None:
    assert len(events) == 7
    assert len(spans) == 5
    span_names = [s.name for s in spans]
    assert span_names == [
    ...
    routing_classifier_span = [s for s in spans if s.name == "routingClassifierTrace"][0]
...
def _assert_invoke_inline_agent(events: list[Any], spans: Sequence[Any]) -> None:
    assert len(events) == 5
    assert len(spans) == 3
    span_names = [s.name for s in spans]
    assert span_names == ["LLM", "orchestrationTrace", "bedrock_agent.invoke_inline_agent"]
    initial_span = [s for s in spans if s.name == "bedrock_agent.invoke_inline_agent"][0]
    ...
    orchestration_span = [s for s in spans if s.name == "orchestrationTrace"][0]
    orchestration_attributes = dict(orchestration_span.attributes or {})
    orchestration_metadata = orchestration_attributes.pop("metadata")
    assert orchestration_metadata is not None
    assert isinstance(orchestration_metadata, str)
...
def test_tool_calls_with_input_params(
    ...
    _assert_tool_calls_with_input_params(events, spans)


@pytest.mark.order(after="test_tool_calls_with_input_params")
async def test_async_tool_calls_with_input_params(
    in_memory_span_exporter: InMemorySpanExporter,
    read_aio_cassette: Any,
) -> None:
    """Async version of test_tool_calls_with_input_params; same cassette and assertions."""
    import aioboto3
    from aioresponses import aioresponses

    with aioresponses() as m:
        read_aio_cassette(str(_CASSETTES_DIR / "test_tool_calls_with_input_params.yaml"), m)
        session = aioboto3.session.Session(region_name="ap-south-1")
...
def test_tool_calls_without_input_params(
    ...
    events = [event for event in response["completion"]]
    spans = in_memory_span_exporter.get_finished_spans()
    _assert_tool_calls_without_input_params(events, spans)


@pytest.mark.order(after="test_tool_calls_without_input_params")
async def test_async_tool_calls_without_input_params(
    in_memory_span_exporter: InMemorySpanExporter,
    read_aio_cassette: Any,
) -> None:
    """Async version of test_tool_calls_without_input_params; same cassette and assertions."""
    import aioboto3
    from aioresponses import aioresponses

    with aioresponses() as m:
        read_aio_cassette(str(_CASSETTES_DIR / "test_tool_calls_without_input_params.yaml"), m)
        session = aioboto3.session.Session(region_name="ap-south-1")
        async with session.client(
            "bedrock-agent-runtime",
            region_name="ap-south-1",
...
def test_knowledge_base_results(
    ...
    events = [event for event in response["completion"]]
    spans = in_memory_span_exporter.get_finished_spans()
    _assert_knowledge_base_results(events, spans)


@pytest.mark.order(after="test_knowledge_base_results")
async def test_async_knowledge_base_results(
    in_memory_span_exporter: InMemorySpanExporter,
    read_aio_cassette: Any,
) -> None:
    """Async version of test_knowledge_base_results; same cassette and assertions."""
    import aioboto3
    from aioresponses import aioresponses

    with aioresponses() as m:
        read_aio_cassette(str(_CASSETTES_DIR / "test_knowledge_base_results.yaml"), m)
        session = aioboto3.session.Session(region_name="ap-south-1")
        async with session.client(
            "bedrock-agent-runtime",
            region_name="ap-south-1",
...
def test_preprocessing_trace(
    ...
    events = [event for event in response["completion"]]
    spans = in_memory_span_exporter.get_finished_spans()
    _assert_preprocessing_trace(events, spans)


@pytest.mark.order(after="test_preprocessing_trace")
async def test_async_preprocessing_trace(
    in_memory_span_exporter: InMemorySpanExporter,
    read_aio_cassette: Any,
) -> None:
    """Async version of test_preprocessing_trace; same cassette and assertions."""
    import aioboto3
    from aioresponses import aioresponses

    with aioresponses() as m:
        read_aio_cassette(str(_CASSETTES_DIR / "test_preprocessing_trace.yaml"), m)
        session = aioboto3.session.Session(region_name="ap-south-1")
        async with session.client(
            "bedrock-agent-runtime",
            region_name="ap-south-1",
...
def test_post_processing_trace(
    ...
    events = [event for event in response["completion"]]
    spans = in_memory_span_exporter.get_finished_spans()
    _assert_post_processing_trace(events, spans)


@pytest.mark.order(after="test_post_processing_trace")
async def test_async_post_processing_trace(
    in_memory_span_exporter: InMemorySpanExporter,
    read_aio_cassette: Any,
) -> None:
    """Async version of test_post_processing_trace; same cassette and assertions."""
    import aioboto3
    from aioresponses import aioresponses

    with aioresponses() as m:
        read_aio_cassette(str(_CASSETTES_DIR / "test_post_processing_trace.yaml"), m)
        session = aioboto3.session.Session(region_name="ap-south-1")
        async with session.client(
            "bedrock-agent-runtime",
            region_name="ap-south-1",
...
def test_agent_call_without_traces(
    ...
    _assert_agent_call_without_traces_span_attributes(dict(spans[0].attributes or {}))


@pytest.mark.order(after="test_agent_call_without_traces")
async def test_async_agent_call_without_traces(
    in_memory_span_exporter: InMemorySpanExporter,
    read_aio_cassette: Any,
) -> None:
    """Async version of test_agent_call_without_traces; same cassette and assertions."""
    import aioboto3
    from aioresponses import aioresponses

    with aioresponses() as m:
        read_aio_cassette(str(_CASSETTES_DIR / "test_agent_call_without_traces.yaml"), m)
...
def test_multi_agent_collaborator(
    ...
    events = [event for event in response["completion"]]
    spans = in_memory_span_exporter.get_finished_spans()
    _assert_multi_agent_collaborator(events, spans)


@pytest.mark.order(after="test_multi_agent_collaborator")
async def test_async_multi_agent_collaborator(
    in_memory_span_exporter: InMemorySpanExporter,
    read_aio_cassette: Any,
) -> None:
    """Async version of test_multi_agent_collaborator; same cassette and assertions."""
    import aioboto3
    from aioresponses import aioresponses
    ...
    session_id = "12345680"
    with aioresponses() as m:
        read_aio_cassette(str(_CASSETTES_DIR / "test_multi_agent_collaborator.yaml"), m)
        session = aioboto3.session.Session(region_name="us-east-1")
        async with session.client(
            "bedrock-agent-runtime",
            region_name="us-east-1",
...
def test_streaming_with_guardrails(in_memory_span_exporter: InMemorySpanExporter) -> None:
    ...
    events = [event for event in response["completion"]]
    spans = in_memory_span_exporter.get_finished_spans()
    _assert_streaming_with_guardrails(events, spans)


@pytest.mark.order(after="test_streaming_with_guardrails")
async def test_async_streaming_with_guardrails(
    in_memory_span_exporter: InMemorySpanExporter,
    read_aio_cassette: Any,
) -> None:
    """Async version of test_streaming_with_guardrails; same cassette and assertions."""
    import aioboto3
    from aioresponses import aioresponses
    ...
    session_id = "12345680"
    with aioresponses() as m:
        read_aio_cassette(str(_CASSETTES_DIR / "test_streaming_with_guardrails.yaml"), m)
        session = aioboto3.session.Session(region_name="us-east-1")
        async with session.client(
            "bedrock-agent-runtime",
            region_name="us-east-1",
...
def test_guardrail_intervention(in_memory_span_exporter: InMemorySpanExporter) -> None:
    ...
    events = [event for event in response["completion"]]
    spans = in_memory_span_exporter.get_finished_spans()
    _assert_guardrail_intervention(events, spans)


@pytest.mark.order(after="test_guardrail_intervention")
async def test_async_guardrail_intervention(
    in_memory_span_exporter: InMemorySpanExporter,
    read_aio_cassette: Any,
) -> None:
    """Async version of test_guardrail_intervention; same cassette and assertions."""
    import aioboto3
    from aioresponses import aioresponses
    ...
    session_id = "12345680"
    with aioresponses() as m:
        read_aio_cassette(str(_CASSETTES_DIR / "test_guardrail_intervention.yaml"), m)
        session = aioboto3.session.Session(region_name="us-east-1")
        async with session.client(
            "bedrock-agent-runtime",
            region_name="us-east-1",
...
@pytest.mark.order(after="test_routing_classifier_trace")
async def test_async_routing_classifier_trace(
    in_memory_span_exporter: InMemorySpanExporter,
    read_aio_cassette: Any,
) -> None:
    """Async version of test_routing_classifier_trace; same cassette and assertions."""
    import aioboto3
    from aioresponses import aioresponses
    ...
    session_id = "12345680"
    with aioresponses() as m:
        read_aio_cassette(str(_CASSETTES_DIR / "test_routing_classifier_trace.yaml"), m)
        session = aioboto3.session.Session(region_name="us-east-1")
        async with session.client(
            "bedrock-agent-runtime",
            region_name="us-east-1",
...
def test_invoke_inline_agent(
    ...
    events = [event for event in response["completion"]]
    spans = in_memory_span_exporter.get_finished_spans()
    _assert_invoke_inline_agent(events, spans)


@pytest.mark.order(after="test_invoke_inline_agent")
async def test_async_invoke_inline_agent(
    in_memory_span_exporter: InMemorySpanExporter,
    read_aio_cassette: Any,
) -> None:
    """Async version of test_invoke_inline_agent; same cassette and assertions."""
    import aioboto3
    from aioresponses import aioresponses

    with aioresponses() as m:
        read_aio_cassette(str(_CASSETTES_DIR / "test_invoke_inline_agent.yaml"), m)
        session = aioboto3.session.Session(region_name="us-east-1")
        async with session.client(
            "bedrock-agent-runtime",
            region_name="us-east-1",
            ...
            response = await client.invoke_inline_agent(**attributes)
            events = []
            async for event in response["completion"]:
                events.append(event)
    spans = in_memory_span_exporter.get_finished_spans()
