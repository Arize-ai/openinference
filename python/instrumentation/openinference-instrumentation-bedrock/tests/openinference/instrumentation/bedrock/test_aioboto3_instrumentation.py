...
class TestAioBotoInstrumentor:
    ...
    async def test_async_retrieve(
        self, in_memory_span_exporter: InMemorySpanExporter, read_aio_cassette: Any
    ) -> None:
        session = aioboto3.session.Session(
            region_name="us-east-1",
            aws_access_key_id="test",
    ...
    async def test_async_invoke_agent_test(
        ...
        with aioresponses() as m:
            ...
            async with session.client("bedrock-agent-runtime") as client:
                ...
                assert starts_with(
                    llm_span_attributes.pop("input.value"), "You are a classifying ag"
                )
