...
This maps directly onto the OTel GenAI semantic conventions message model: an inline data URI corresponds to a `blob` part, while an externalized reference corresponds to a `uri` part. The same split applies to `message_content.audio.audio.url` and `message_content.video.video.url` (`modality` `audio` or `video`).
...
When `OPENINFERENCE_HIDE_INPUT_AUDIO` is true, replace `message_content.audio.audio.url` (and optional mime type and transcript) on input messages with `"__REDACTED__"` or drop them. `OPENINFERENCE_HIDE_OUTPUT_AUDIO` does the same for output messages.
