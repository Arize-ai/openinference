...
| `message_content.type`                         | String                      | `"text"`, `"image"`, or `"audio"`                                                                                             | The type of the content, such as `"text"`, `"image"`, or `"audio"`.                                                                                                                 |
...
| `message_content.audio.url`                    | String                      | `"https://example.com/audio.mp3"`, `"data:audio/wav;base64,..."`                                                              | URL of the audio asset, or a `data:` URI carrying inline base64 bytes. Applies when `message_content.type` is `"audio"`.                                                            |
| `message_content.audio.mime_type`              | String                      | `"audio/mpeg"`, `"audio/wav"`                                                                                                 | Optional MIME type of the audio asset. Applies when `message_content.type` is `"audio"`.                                                                                            |
| `message_content.audio.transcript`             | String                      | `"Hello, how are you?"`                                                                                                       | Rendered text of the audio content. Applies when `message_content.type` is `"audio"`. Optional on input audio, recommended on assistant audio outputs.                              |
| `metadata`                                     | JSON String                 | `"{'author': 'John Doe', 'date': '2023-09-09'}"`                                                                              | Metadata associated with a span                                                                                                                                                     |
| `openinference.end_reason`                     | String                      | `"completed"` or `"interrupted_by_user"`                                                                                      | Optional. Describes how a span ended from the observer's perspective (lifecycle — e.g., user interruption, programmatic cancellation, session close). Distinct from `llm.finish_reason`, which is the provider-reported reason the *model* stopped generating tokens. Applies to any span kind. See [End Reason Values](#end-reason-values). |
...
The `openinference.end_reason` attribute is optional, but when emitted it describes how a span ended from the observer's perspective. The following values are well-known; if one of them applies, the respective value MUST be used.
...
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.type` - Content type (`"text"`, `"image"`, or `"audio"`)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.text` - Text content (when `type = "text"`)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.image.image.url` - Image URL or `data:` URI (when `type = "image"`)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.audio.url` - Audio URL or `data:` URI (when `type = "audio"`)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.audio.mime_type` - Audio MIME type (optional; when `type = "audio"`)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.audio.transcript` - Audio transcript (when `type = "audio"`)
