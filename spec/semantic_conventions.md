...
| `message_content.audio`                        | Audio Object                | `{"audio.url": "https://example.com/audio.mp3"}`                                                                               | The audio content of the message, if the type is "audio". Nested leaves are `audio.url` and `audio.transcript`. Consumers infer MIME type from the URL path extension or a data URI prefix. |
...
| `audio.url`                                    | String                      | `https://storage.com/buckets/1/file.wav`                                                                                      | The url to an audio file (e.x. cloud storage). Also composed under `message_content.audio` and as span-root `input.audio.url` / `output.audio.url`. Consumers infer MIME type from the path extension or a data URI prefix. |
...
- `llm.<input|output>_messages.<messageIndex>.message.contents.<contentIndex>.message_content.audio.audio.url` - Audio URL or base64 data URI. Infer MIME type from the path extension or data URI prefix. Do not emit `audio.mime_type`.
...
- `llm.<input|output>_messages.<messageIndex>.message.contents.<contentIndex>.message_content.video.video.url` - Video URL, object-store URI, or base64 data URI. Infer MIME type from the path extension or data URI prefix. Do not emit `video.mime_type`.
...
- `input.audio.url` - User audio URL or WAV data URI. Infer MIME type from the path extension or data URI prefix. Do not emit `audio.mime_type`.
- `input.audio.transcript` - User audio transcript
- `output.audio.url` - Assistant audio URL or WAV data URI. Infer MIME type from the path extension or data URI prefix. Do not emit `audio.mime_type`.
