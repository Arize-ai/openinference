...
| `DECISION`      | A span that represents a call to a decision model, which scores or selects among candidate options provided in the request rather than generating free-form text. For example, a Decision span could represent choosing a route, judging a condition, or scoring an item against a rubric.                                                                                                                                                  |
...
| ---------------------------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `annotation.annotator_kind`                    | String                      | `"LLM"`                                                                                                                       | Judge kind: `HUMAN`, `LLM`, `CODE`, or custom                                                                                                                                       |
| `annotation.explanation`                       | String                      | `"The claim is not supported by the context."`                                                                                | Reason or evidence for the result                                                                                                                                                   |
| `annotation.identifier`                        | String                      | `"judge-v2"`                                                                                                                  | Stable result ID for the same name and target                                                                                                                                       |
| `annotation.label`                             | String                      | `"hallucinated"`                                                                                                              | Categorical result                                                                                                                                                                  |
| `annotation.metadata`                          | JSON String                 | `"{\"evaluator\":\"judge-v2\"}"`                                                                                           | Additional result or judge data                                                                                                                                                    |
| `annotation.name`                              | String                      | `"hallucination"`                                                                                                             | Criterion or metric name                                                                                                                                                            |
| `annotation.score`                             | Integer/Float               | `1`                                                                                                                           | Numeric result                                                                                                                                                                      |
...
| `evaluation.annotator_kind`                    | String                      | `"HUMAN"`                                                                                                                     | Judge kind: `HUMAN`, `LLM`, `CODE`, or custom                                                                                                                                       |
| `evaluation.explanation`                       | String                      | `"The response directly answers the question."`                                                                               | Reason or evidence for the result                                                                                                                                                   |
| `evaluation.identifier`                        | String                      | `"reviewer-42"`                                                                                                               | Stable result ID for the same name and target                                                                                                                                       |
| `evaluation.label`                             | String                      | `"relevant"`                                                                                                                  | Categorical result                                                                                                                                                                  |
| `evaluation.metadata`                          | JSON String                 | `"{\"rubric_version\":\"2\"}"`                                                                                            | Additional result or judge data                                                                                                                                                    |
| `evaluation.name`                              | String                      | `"relevance"`                                                                                                                 | Criterion or metric name                                                                                                                                                            |
| `evaluation.score`                             | Integer/Float               | `0.95`                                                                                                                        | Numeric result                                                                                                                                                                      |
| `evaluations`                                  | List of objects<sup>†</sup> | `[{"evaluation.name": "relevance", "evaluation.score": 0.95}]`                                                             | Feedback using evaluation or eval terminology, flattened as `evaluations.0.evaluation.*`. See [Annotations and Evaluations](./annotations.md)                                    |
| `trace.evaluations`                            | List of objects<sup>†</sup> | `[{"evaluation.name": "retrieval_quality", "evaluation.score": 0.92}]`                                                       | Trace-scoped feedback using evaluation terminology, flattened as `trace.evaluations.0.evaluation.*`                                                                               |
| `session.evaluations`                          | List of objects<sup>†</sup> | `[{"evaluation.name": "conversational_coherence", "evaluation.label": "coherent"}]`                                        | Session-scoped feedback using evaluation terminology, flattened as `session.evaluations.0.evaluation.*`; requires `session.id`                                                    |
...
| `input.images`                                 | List of objects<sup>†</sup> | `[{"image.url": "https://sample-link-to-image.jpg"}]`                                                                         | Image inputs to an operation, independent of span kind. Uses flattened attributes with indexed prefixes (e.g., `input.images.0.image.url`). See [Multimodal Attributes](./multimodal_attributes.md#span-kind-independent-images) |
...
| `llm.finish_reason`                            | String                      | `"stop"`, `"length"`                                                                                                          | The reason the model stopped generating tokens.                                                                                                                                     |
...
| `llm.tools`                                    | List of objects<sup>†</sup> | `[{"tool.name": "get_weather", "tool.json_schema": "{\"type\": \"function\", \"function\": {\"name\": \"get_weather\"}}"}]`   | List of tools that are advertised to the LLM to be able to call. Uses flattened attributes with indexed prefixes (e.g., `llm.tools.0.tool.json_schema`, `llm.tools.0.tool.name`, `llm.tools.0.tool.description`) |
| `message.content`                              | String                      | `"What's the weather today?"`                                                                                                 | The content of a message in a chat                                                                                                                                                  |
| `message.contents`                             | List of objects<sup>†</sup> | `[{"message_content.type": "text", "message_content.text": "Hello"}, ...]`                                                    | The message contents to the llm, it is an array of `message_content` objects. Use this form when a message has multiple content items or when item ordering matters.                |
...
| `message.name`                                 | String                      | `"multiply"`                                                                                                                  | The name of the function or tool that produced a tool/function role message. Used when `message.role` is `"tool"` or `"function"` to identify the origin.                           |
...
| `message_content.type`                         | String                      | `"text"`, `"image"`, `"audio"`, `"video"`, `"reasoning"`, or `"tool_use"`                                                      | The type of the content. Use `"reasoning"` for reasoning/thinking content, including Anthropic `redacted_thinking` blocks. Use `"tool_use"` when a provider returns a tool call as an ordered content part. Use `"video"` for video parts. |
| `message_content.text`                         | String                      | `"This is a sample text"`                                                                                                     | The text content of the message. Used when `message_content.type` is `"text"` or `"reasoning"` (visible reasoning or summary text). For OpenAI `summary_text` arrays, concatenate the array items into this single text value for now. |
| `message_content.image`                        | Image Object                | `{"image.url": "https://sample-link-to-image.jpg"}`                                                                           | The image content of the message, if the type is "image".                                                                                                                           |
| `message_content.audio`                        | Audio Object                | `{"audio.url": "https://example.com/audio.mp3"}`                                                                             | The audio content of the message, if the type is "audio". Nested leaves are `audio.url` and `audio.transcript`.                                                                   |
| `message_content.video`                        | Video Object                | `{"video.url": "gs://bucket/clip.mp4"}`                                                                                       | The video content of the message, if the type is "video". The nested leaf is `video.url`. Consumers infer MIME type from the URL path extension or a data URI prefix.              |
| `message_content.id`                           | String                      | `"rs_abc123"`                                                                                                                 | Provider-assigned identifier for this message content item. For OpenAI Responses reasoning items, this maps to `ResponseReasoningItem.id` and should be preserved for stateless replay. |
| `message_content.signature`                    | String                      | `"EuYBCkQYAiJA..."`                                                                                                           | Opaque vendor-issued signature captured verbatim. Maps to provider `signature` fields, and to Gemini `thoughtSignature` fields when the signature is attached to a non-tool content part such as text. |
...
| `tool.description`                             | String                      | `"An API to get weather data."`                                                                                               | Description of the tool's purpose and functionality. On a `TOOL` span, the tool being invoked; under `llm.tools.<index>`, an advertised tool definition                             |
| `tool.json_schema`                             | JSON String                 | `"{'type': 'function', 'function': {'name': 'get_weather'}}"`                                                                 | The json schema of a tool input                                                                                                                                                     |
| `tool.name`                                    | String                      | `"WeatherAPI"`                                                                                                                | The name of the tool. On a `TOOL` span, the tool being invoked; under `llm.tools.<index>`, the name of an advertised tool definition                                                |
| `tool.id`                                      | String                      | `"call_62136355"`                                                                                                             | The identifier for the result of the tool call (corresponding to `tool_call.id`)                                                                                                    |
...
| `audio.mime_type`                              | String                      | `audio/mpeg`                                                                                                                  | Legacy. Infer the type from `audio.url` instead; new instrumentors should not emit this.                                                                                                                   |
...
- `trace.annotations.<index>.annotation.<attribute>` - Trace-scoped annotation
- `trace.evaluations.<index>.evaluation.<attribute>` - Trace-scoped evaluation
- `session.annotations.<index>.annotation.<attribute>` - Session-scoped annotation
- `session.evaluations.<index>.evaluation.<attribute>` - Session-scoped evaluation

All forms share the same result fields. The optional `trace` and `session` prefixes widen the target from one span to
one trace or session. See [Annotations and Evaluations](./annotations.md).
...
For messages containing multiple content items (text, images, audio, video, reasoning, or provider-native tool-use parts):

- `llm.<input|output>_messages.<messageIndex>.message.contents.<contentIndex>.message_content.type` - Content type (`"text"`, `"image"`, `"audio"`, `"video"`, `"reasoning"`, or `"tool_use"`)
...
- `llm.<input|output>_messages.<messageIndex>.message.contents.<contentIndex>.message_content.audio.audio.url` - Audio URL or base64 data URI
- `llm.<input|output>_messages.<messageIndex>.message.contents.<contentIndex>.message_content.audio.audio.transcript` - Transcript for this audio part, when present
- `llm.<input|output>_messages.<messageIndex>.message.contents.<contentIndex>.message_content.video.video.url` - Video URL, object-store URI, or base64 data URI. Infer MIME type from the path extension or data URI prefix. There is no video MIME type attribute.
...
- `llm.<input|output>_messages.<messageIndex>.message.contents.<contentIndex>.message_content.encrypted_content` - OpenAI `encrypted_content`
- `llm.<input|output>_messages.<messageIndex>.message.contents.<contentIndex>.tool_call.id` - Tool-call id when `message_content.type` is `"tool_use"` and ordering relative to adjacent content items must be preserved
...
- `llm.output_messages.<messageIndex>.message.tool_calls.<toolCallIndex>.tool_call.reasoning_signature` - Provider reasoning signature attached to the tool call, such as Gemini `thoughtSignature`

#### Available Tools

- `llm.tools.<index>.tool.json_schema` - Complete JSON schema of the tool, including type, function definition, and parameters
- `llm.tools.<index>.tool.name` - Name of the tool, i.e. the identifier the model uses to call it
- `llm.tools.<index>.tool.description` - Description of the tool, i.e. the text the model uses to decide whether to call it
