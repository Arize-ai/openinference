...
| `latency.streaming.time_to_first_response_ms`  | Number                      | `100`                                                                                                                         | Time, in milliseconds, from the start of the request to the first byte of the model's response reaching the client. For non-streaming responses this is equivalent to the span duration; for streaming responses (including streamed audio in voice conversations) this measures how long the client waited before receiving any output. |
...
| `llm.end_reason`                               | String                      | `"completed"` or `"interrupted_by_user"`                                                                                      | The reason an LLM span ended. Applies to any LLM call, including voice-conversation rounds and agent-harness invocations. See [LLM End Reason Values](#llm-end-reason-values) for the enum. |
...
| `message_content.audio.voice_id`               | String                      | `"marin"` or `"alloy"`                                                                                                        | Provider-defined voice identifier. Recommended on assistant audio outputs.                                                                                                          |
...
The `llm.end_reason` attribute identifies how an LLM span ended. It applies to any LLM call — voice-conversation rounds (set on the round `LLM` span on the native path or the round `CHAIN` span on the cascaded path), child STT / thinker / TTS spans within a cascaded voice round, and ordinary single-shot LLM invocations (including agent-harness calls where a user may cancel mid-generation). The following values are well-known; if one of them applies, the respective value MUST be used.
...
| `error`               | Provider or transport error occurred mid-span.                                                               |
| `timeout`             | Span exceeded a configured time limit.                                                                       |
...
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.audio.voice_id` - Provider-defined voice identifier (assistant audio outputs)
