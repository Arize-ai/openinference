...
| `message_content.file.voice_name`              | String                      | `"alloy"` or `"verse"`                                                                                                        | Provider-defined TTS voice preset used to synthesize the audio (e.g., OpenAI `alloy`/`verse`). A preset identifier, not a speaker/diarization id and not a user identity. Applies when `message_content.type` is `"audio"` on assistant output messages. |
...
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.file.voice_name` - TTS voice preset (applies when `type = "audio"` on assistant output messages)
