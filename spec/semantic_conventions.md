...
| `message_content.type`                         | String                      | `"text"`, `"image"`, `"audio"`, or `"document"`                                                                               | The semantic category of the content. Closed enum of well-known values; new values require a spec update. Discriminates which `file.*` fields apply.                                |
| `message_content.text`                         | String                      | `"This is a sample text"`                                                                                                     | The text content of the message, when `message_content.type` is `"text"`.                                                                                                           |
| `message_content.image`                        | Image Object                | `{"image.url": "https://sample-link-to-image.jpg"}`                                                                           | **Deprecated** — use `message_content.file.url` with `message_content.type = "image"` instead. Previously used to attach image content to a message.                                |
| `message_content.image.image.url`              | String                      | `"https://sample-link-to-image.jpg"`                                                                                          | **Deprecated** — use `message_content.file.url` with `message_content.type = "image"` instead. Pre-existing double-nested image URL shape still accepted by consumers.              |
| `message_content.file.url`                     | String                      | `"https://example.com/image.jpg"`                                                                                             | URL of the file asset, or a `data:` URI carrying inline base64 bytes. Applies to any non-text `message_content.type`.                                                               |
| `message_content.file.mime_type`               | String                      | `"image/png"`, `"audio/wav"`, `"application/pdf"`                                                                             | Optional MIME type of the file asset. Recommended for `data:` URIs and URLs with ambiguous extensions.                                                                              |
| `message_content.file.name`                    | String                      | `"travel-policy.pdf"`                                                                                                         | Optional filename or label for the file asset.                                                                                                                                      |
| `message_content.file.file_id`                 | String                      | `"file-abc123XYZ"`                                                                                                            | Optional opaque provider file ID (e.g., OpenAI Files, Anthropic Files, Gemini File API). Used when no fetchable URL is available; SHOULD NOT be placed in `file.url`.               |
| `message_content.file.transcript`              | String                      | `"Hello, how are you?"`                                                                                                       | Rendered text of the audio content. Applies when `message_content.type` is `"audio"`. Optional on input audio, recommended on assistant audio outputs.                              |
| `metadata`                                     | JSON String                 | `"{'author': 'John Doe', 'date': '2023-09-09'}"`                                                                              | Metadata associated with a span                                                                                                                                                     |
| `openinference.end_reason`                     | String                      | `"completed"` or `"interrupted_by_user"`                                                                                      | Optional. Describes how a span ended from the observer's perspective (lifecycle — e.g., user interruption, programmatic cancellation, session close). Distinct from `llm.finish_reason`, which is the provider-reported reason the *model* stopped generating tokens. Conditions already representable via OTel span `status=ERROR` are not enumerated here. Applies to any span kind. See [End Reason Values](#end-reason-values). |
...
| `audio.url`                                    | String                      | `https://storage.com/buckets/1/file.wav`                                                                                      | **Deprecated** — use `message_content.file.url` on a content block with `message_content.type = "audio"` instead. The url to an audio file (e.x. cloud storage)                     |
| `audio.mime_type`                              | String                      | `audio/mpeg`                                                                                                                  | **Deprecated** — use `message_content.file.mime_type` on a content block with `message_content.type = "audio"` instead. The mime type of the audio file                             |
| `audio.transcript`                             | String                      | `"Hello, how are you?"`                                                                                                       | **Deprecated** — use `message_content.file.transcript` on a content block with `message_content.type = "audio"` instead. The transcript of the audio file (e.x. whisper transcription) |
...
Attributes and shapes that are renamed, restructured, or replaced are marked **Deprecated** in this spec, with a pointer to the replacement. Deprecated shapes are subject to the following support window:

- **Instrumentors** — SHOULD migrate to the replacement shape promptly. New instrumentation MUST emit the replacement shape.
- **Consumers** (backends, UIs, dashboards) — MUST continue to accept the deprecated shape for at least **12 months** after the deprecation marker first appears in a tagged spec release. Removal from a consumer before that window is a breaking change.
- **Spec** — MAY remove deprecated entries after the 12-month support window has elapsed. Spec changes that drop a deprecated entry SHOULD reference the release in which the deprecation was introduced.

The 12-month window starts at the spec release that introduces the deprecation marker, not the release that introduces the replacement (these may differ when a replacement ships before its predecessor is formally deprecated).

## Span Events

In addition to attributes, OpenInference reserves a small number of well-known **span event** names. Span events carry a timestamp natively, making them the right shape for marking instants within a span (e.g., "first streamed chunk arrived") rather than aggregate values measured at span end.

| Event Name        | When to Emit                                                                                                                                                                                                                                                                                                                                                                       |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `llm.first_token` | Recommended for streaming LLM spans. Emit at the moment the first streamed chunk (token, byte, or audio frame) of the model's response reaches the client. The event's timestamp, minus the span start time, gives Time-To-First-Token (TTFT). Omit for non-streaming responses — span duration already captures end-to-end latency in that case. Carries no attributes by default. |
...
For messages containing multiple content items (text, images, audio, documents):

- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.type` - Content type (`"text"`, `"image"`, `"audio"`, or `"document"`)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.text` - Text content (when `type = "text"`)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.file.url` - File URL or `data:` URI (non-text types)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.file.mime_type` - File MIME type (optional)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.file.name` - Filename or label (optional)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.file.file_id` - Opaque provider file ID (optional; used when no fetchable URL is available)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.file.transcript` - Audio transcript (applies when `type = "audio"`)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.image.image.url` - **Deprecated** double-nested image URL; still accepted, prefer `message_content.file.url` with `type = "image"`
