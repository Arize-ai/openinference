...
| `message_content.type`                         | String                      | `"text"`, `"image"`, `"audio"`, or `"reasoning"`                                                                              | The type of the content (e.g. `"text"`, `"image"`, `"audio"`, `"reasoning"`). Use `"reasoning"` for reasoning/thinking content, including Anthropic `redacted_thinking` blocks.     |
...
| `message_content.signature`                    | String                      | `"EuYBCkQYAiJA..."`                                                                                                           | Opaque vendor-issued signature captured verbatim. Maps to provider `signature` and `thinkingSignature` fields.                                                                      |
...
| `message_content.encripted_content`            | String                      | `"gAAAAA...=="`                                                                                                               | OpenAI `encrypted_content` captured verbatim.                                                                                                                                       |
...
| `tool_call.signature`                          | String                      | `"CiQB..."`                                                                                                                   | Opaque vendor-issued echo token attached to a tool call. Used by Gemini, which puts `thoughtSignature` on `functionCall` parts rather than on the reasoning summary.                |
...
For messages containing multiple content items (text, images, audio, reasoning):

- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.text` - Text content item; OpenAI `summary_text` arrays are concatenated into this value for now
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.type` - Content type ("text", "image", "audio", "reasoning")
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.image.image.url` - Image URL or base64 data
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.signature` - Provider `signature` or `thinkingSignature`
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.data` - Anthropic `redacted_thinking.data`
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.encripted_content` - OpenAI `encrypted_content`
...
- `llm.output_messages.<messageIndex>.message.tool_calls.<toolCallIndex>.tool_call.signature` - Provider signature attached to the tool call, such as Gemini `thoughtSignature`
