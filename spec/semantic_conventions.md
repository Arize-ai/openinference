...
| `llm.end_reason`                               | String                      | `"completed"` or `"interrupted_by_user"`                                                                                      | Optional. Describes how the LLM span ended from the observer's perspective (lifecycle — e.g., user interruption, programmatic cancellation, session close). Distinct from `llm.finish_reason`, which is the provider-reported reason the model stopped generating tokens (`stop`, `length`, `tool_calls`). Conditions already representable via OTel span `status=ERROR` are not enumerated here. See [LLM End Reason Values](#llm-end-reason-values) for the enum. |
...
| `message_content.type`                         | String                      | `"text"`, `"image"`, `"audio"`, or `"document"`                                                                               | The type of the content, such as "text", "image", "audio", or "document".                                                                                                           |
| `message_content.text`                         | String                      | `"This is a sample text"`                                                                                                     | The text content of the message, if the type is "text".                                                                                                                             |
| `message_content.image`                        | Image Object                | `{"image.url": "https://sample-link-to-image.jpg"}`                                                                           | **Deprecated** — use the single-nested `message_content.image.url` / `message_content.image.mime_type` attributes instead. The double-nested `message_content.image.image.url` shape is still accepted but will be phased out. |
| `message_content.image.url`                    | String                      | `"https://sample-link-to-image.jpg"`                                                                                          | The link to the image content. Replaces the double-nested `message_content.image.image.url` shape.                                                                                  |
| `message_content.image.mime_type`              | String                      | `"image/png"` or `"image/jpeg"`                                                                                               | The MIME type of the image content.                                                                                                                                                 |
| `message_content.audio.url`                    | String                      | `"https://storage.com/buckets/1/file.wav"`                                                                                    | The link to the audio asset, for content blocks where `message_content.type` is `"audio"`.                                                                                          |
| `message_content.audio.mime_type`              | String                      | `"audio/wav"` or `"audio/mpeg"`                                                                                               | The MIME type of the audio content.                                                                                                                                                 |
| `message_content.audio.transcript`             | String                      | `"Hello, how are you?"`                                                                                                       | The transcript of the audio content. Optional on input audio, recommended on assistant audio outputs.                                                                               |
| `message_content.audio.voice_name`             | String                      | `"alloy"` or `"verse"`                                                                                                        | The provider-defined voice preset used to synthesize the audio (e.g., OpenAI `alloy`/`verse`, Anthropic-style named voices). This is a TTS preset identifier, not a speaker/diarization id and not a user identity. Recommended on assistant audio outputs.                                            |
| `message_content.document.url`                 | String                      | `"gs://docs-bucket/policies/travel-policy.pdf"`                                                                               | The link to the document asset, for content blocks where `message_content.type` is `"document"`.                                                                                    |
| `message_content.document.mime_type`           | String                      | `"application/pdf"`                                                                                                           | The MIME type of the document content.                                                                                                                                              |
| `message_content.document.name`                | String                      | `"travel-policy.pdf"`                                                                                                         | Filename or label for the document.                                                                                                                                                 |
...
| `audio.url`                                    | String                      | `https://storage.com/buckets/1/file.wav`                                                                                      | **Deprecated** — use `message_content.audio.url` on a typed content block instead. The url to an audio file (e.x. cloud storage)                                                    |
| `audio.mime_type`                              | String                      | `audio/mpeg`                                                                                                                  | **Deprecated** — use `message_content.audio.mime_type` on a typed content block instead. The mime type of the audio file (e.x. `audio/mpeg`, `audio/wav` )                          |
| `audio.transcript`                             | String                      | `"Hello, how are you?"`                                                                                                       | **Deprecated** — use `message_content.audio.transcript` on a typed content block instead. The transcript of the audio file (e.x. whisper transcription)                             |
...
### LLM End Reason Values

The `llm.end_reason` attribute is optional, but when emitted it describes how an LLM span ended from the observer's perspective. It complements `llm.finish_reason`: `finish_reason` reports why the *model* stopped generating tokens (provider-reported), while `end_reason` reports the *span lifecycle* outcome. It applies to any LLM call, including voice-conversation rounds and ordinary single-shot LLM invocations (such as agent-harness calls where a user may cancel mid-generation). Conditions already representable via OTel span `status=ERROR` (provider/transport errors, timeouts) are not enumerated here — set the span status instead. The following values are well-known; if one of them applies, the respective value MUST be used.
...
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.type` - Content type ("text", "image", "audio", "document")
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.image.url` - Image URL
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.image.mime_type` - Image MIME type
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.image.image.url` - **Deprecated** double-nested image URL; still accepted, prefer `message_content.image.url`
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.audio.url` - Audio URL
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.audio.mime_type` - Audio MIME type
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.audio.transcript` - Audio transcript
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.audio.voice_name` - Provider-defined TTS voice preset (assistant audio outputs)
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.document.url` - Document URL
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.document.mime_type` - Document MIME type
- `llm.input_messages.<messageIndex>.message.contents.<contentIndex>.message_content.document.name` - Document filename or label
