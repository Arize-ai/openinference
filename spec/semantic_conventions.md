...
| `llm.voice_name`                               | String                      | `"alloy"` or `"verse"`                                                                                                        | Optional. The provider-defined TTS voice preset requested for audio responses (e.g., OpenAI `alloy`/`verse`). A request-side invocation parameter — distinct from any speaker/diarization id or user identity. Use this when the voice is selected at the request level rather than per-message. |
