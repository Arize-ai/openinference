...
| `message_content.type`                         | String                      | `"text"`, `"image"`, `"audio"`, or `"reasoning"`                                                                              | The type of the content (e.g. `"text"`, `"image"`, `"audio"`, `"reasoning"`).                                                                                                       |
| `message_content.id`                           | String                      | `"rs_abc123"` or `"msg_def456"`                                                                                               | Vendor-assigned identifier for the content block, when one exists. Required on stateless echo for OpenAI Responses (`rs_*` for reasoning items, `msg_*` for message items). Unset for Anthropic/Gemini. |
| `message_content.text`                         | String                      | `"This is a sample text"`                                                                                                     | The text content of the message. Used when `message_content.type` is `"text"` or `"reasoning"` (visible chain-of-thought / summary text).                                           |
...
| `message_content.signature`                    | String                      | `"EuYBCkQYAiJA..."`                                                                                                           | Opaque vendor-issued echo token captured verbatim. Maps to OpenAI `encrypted_content`, Anthropic `signature` (or `data` for redacted blocks), or Gemini `thoughtSignature`. Required on stateless multi-turn echo. |
