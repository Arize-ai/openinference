...
PR or summary. Two read-back paths return the same span JSON: the `px` CLI with `jq`, or the
Phoenix MCP server (`mcp__plugin_arize-phoenix_phoenix__execute`) when it is connected. Pick
whichever is available; recipes for both live in [REFERENCE.md](REFERENCE.md).
