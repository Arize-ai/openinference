...
function displayRecordings(recordings) {
  ...
  console.log(`  ${colors.bold}Total: ${recordings.files.length} files, ${totalSizeKB} KB${colors.reset}\n`);
}
...
function removeRecordings(recordings, options = {}) {
  ...
  filesToRemove.forEach(file => {
    try {
      ...
      results.removed++;
      console.log(`  ${colors.green}✅ Removed: ${file.relativePath}${colors.reset}`);
    } catch (error) {
      results.errors.push({ file: file.relativePath, error: error.message });
      console.log(`  ${colors.red}❌ Failed to remove: ${file.relativePath} (${error.message})${colors.reset}`);
    }
  });
...
async function interactiveMode() {
  ...
  try {
    console.log(`${colors.bold}${colors.blue}🗑️  Interactive Recording Cleanup${colors.reset}\n`);
    ...
    switch (choice) {
      case '1':
        const confirmAll = await askQuestion(rl, 
      ...
      case '2':
        ...
        const directories = [...new Set(recordings.files.map(f => 
        ...
        directories.forEach((dir, index) => {
          ...
          console.log(`  ${colors.blue}${index + 1}.${colors.reset} ${dir}/ (${filesInDir} files)`);
        });
        ...
        if (dirIndex >= 0 && dirIndex < directories.length) {
          ...
          const confirmDir = await askQuestion(rl, 
          ...
          if (confirmDir === 'y' || confirmDir === 'yes') {
            ...
            const results = removeRecordings(recordings, { directory: selectedDir });
...
function commandLineMode() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    ...
    console.log('  npm run test:clear-recordings -- --all        # Remove all files');
    console.log('  npm run test:clear-recordings -- --dir <name> # Remove directory');
    console.log('  npm run test:clear-recordings -- --dry-run    # Show what would be removed');
    console.log('\nOptions:');
    console.log('  --all             Remove all recording files');
    console.log('  --dir <name>      Remove files from specific directory');
    console.log('  --dry-run         Show files that would be removed without deleting');
  ...
  const directory = dirIndex >= 0 && dirIndex + 1 < args.length ? args[dirIndex + 1] : null;
  
  if (isDryRun) {
    console.log(`${colors.bold}${colors.blue}🔍 Dry Run - Files that would be removed:${colors.reset}\n`);
    displayRecordings(recordings);
    ...
    if (directory) {
      ...
      console.log(`${colors.blue}Would remove ${filteredFiles.length} files from ${directory}/${colors.reset}`);
    } else if (isAll) {
      console.log(`${colors.blue}Would remove all ${recordings.files.length} files${colors.reset}`);
    }
    return;
  }
  ...
  if (isAll || directory) {
    ...
    if (!isForce) {
      console.log(`${colors.yellow}⚠️  This will remove ${filesToRemove.length} files (${description})${colors.reset}`);
      console.log(`${colors.yellow}Use --force to skip this confirmation${colors.reset}`);
      return;
    }
