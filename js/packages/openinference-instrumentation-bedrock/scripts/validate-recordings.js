...
const validationRules = {
  ...
  requiredFields: ["scope", "method", "path", "status", "response"],
...
function validateSingleRecording(recording, index, results, filePath) {
  ...
  validationRules.requiredFields.forEach((field) => {
    if (!recording.hasOwnProperty(field)) {
      results.errors.push(
        `${recordingPrefix}: Missing required field '${field}'`,
      );
      results.valid = false;
    }
  });
  ...
  if (
    recording.method &&
    !validationRules.validMethods.includes(recording.method)
  ) {
    results.warnings.push(
      `${recordingPrefix}: Unexpected HTTP method '${recording.method}'`,
    );
  }
  ...
  if (
    recording.status &&
    !validationRules.validStatusCodes.includes(recording.status)
  ) {
    results.warnings.push(
      `${recordingPrefix}: Unexpected status code ${recording.status}`,
    );
  }

  // Validate endpoint
  if (recording.path) {
    const isValidEndpoint = validationRules.validEndpoints.some(
      (pattern) =>
        pattern.test(recording.scope + recording.path) ||
        pattern.test(recording.path),
    );
    if (!isValidEndpoint) {
      results.warnings.push(
        `${recordingPrefix}: Unexpected endpoint '${recording.path}'`,
      );
    }
  }
  ...
  validationRules.sensitivePatterns.forEach((pattern) => {
    ...
    if (matches) {
      results.errors.push(
        `${recordingPrefix}: Found potential sensitive data: ${matches.length} matches for pattern ${pattern}`,
      );
      results.valid = false;
    }
  });
  ...
  try {
    ...
    if (typeof recording.response === "string") {
      try {
        ...
        results.warnings.push(
          `${recordingPrefix}: Response body is not valid JSON`,
        );
        return;
      }
    } else {
    ...
    if (recording.path) {
      if (
        recording.path.includes("/invoke") &&
        !recording.path.includes("stream")
      ) {
    ...
    results.warnings.push(
      `${recordingPrefix}: Error validating response structure: ${error.message}`,
    );
  }
}
...
function validateInvokeModelResponse(responseBody, prefix, results) {
  ...
  if (responseBody.content) {
    ...
    if (Array.isArray(responseBody.content)) {
      responseBody.content.forEach((block, i) => {
        if (!block.type) {
          results.warnings.push(
            `${prefix}: Content block ${i} missing 'type' field`,
          );
        }
      });
    }
  ...
  if (responseBody.content && Array.isArray(responseBody.content)) {
    const hasToolUse = responseBody.content.some(
  ...
  if (responseBody.output) {
    ...
    if (responseBody.output.message && responseBody.output.message.content) {
      ...
      if (Array.isArray(content)) {
        results.info.push(
          `${prefix}: Contains ${content.length} content blocks`,
        );
      }
    }
  }
...
function printSummary(results) {
  ...
  console.log(
    `  Valid files: ${colors.green}${results.validFiles}${colors.reset}`,
  );
  console.log(
    `  Invalid files: ${colors.red}${results.invalidFiles}${colors.reset}`,
  );
...
function main() {
  console.log(
    `${colors.bold}${colors.blue}🔍 Recording Validation Tool${colors.reset}\n`,
  );
