...
function validateCredentialFormat() {
  ...
  if (!/^AKIA[0-9A-Z]{16}$/.test(accessKeyId) && !/^ASIA[0-9A-Z]{16}$/.test(accessKeyId)) {
    issues.push('AWS_ACCESS_KEY_ID format may be invalid (expected AKIA... or ASIA...)');
  }
  
  if (secretAccessKey.length !== 40) {
    issues.push('AWS_SECRET_ACCESS_KEY length may be invalid (expected 40 characters)');
  }
...
function ensureRecordingDirectories() {
  ...
  const subdirs = ['basic', 'tools', 'streaming', 'errors', 'multimodal', 'converse', 'agent'];
...
function getRecordingMode() {
  const mode = process.env.BEDROCK_RECORD_MODE || process.env.NOCK_RECORD_MODE || 'replay';
  const validModes = ['record', 'replay', 'lockdown'];
  
  if (!validModes.includes(mode)) {
    console.warn(`${colors.yellow}⚠️  Invalid recording mode: ${mode}. Using 'replay'${colors.reset}`);
    return 'replay';
  }
...
function printStatus() {
  ...
  if (credentialCheck.hasCredentials) {
    console.log(`  ${colors.green}✅ Found required credentials${colors.reset}`);
    ...
    console.log(`  ${colors.red}❌ Missing required credentials${colors.reset}`);
  ...
  if (credentialCheck.hasCredentials) {
    ...
    if (validation.valid) {
      console.log(`  ${colors.green}✅ Credentials appear valid${colors.reset}`);
    } else {
  ...
  const modeColor = environment.mode === 'record' ? colors.green : 
                   environment.mode === 'replay' ? colors.blue : colors.yellow;
  console.log(`  ${modeColor}📹 ${environment.mode.toUpperCase()}${colors.reset}`);
  ...
  if (environment.mode === 'record') {
    console.log(`  ${colors.green}🎥 Ready to record new API calls${colors.reset}`);
    ...
    console.log(`  ${colors.yellow}🔒 Lockdown mode - no network calls allowed${colors.reset}`);
  }
  
  // Next steps
  console.log(`\n${colors.bold}Next Steps:${colors.reset}`);
  if (!credentialCheck.hasCredentials && environment.mode === 'record') {
    console.log(`  ${colors.red}1. Set AWS credentials to enable recording${colors.reset}`);
  ...
  console.log(`  ${colors.blue}2. Clear recordings: npm run test:clear-recordings${colors.reset}`);
  console.log(`  ${colors.blue}3. Validate recordings: npm run test:validate-recordings${colors.reset}`);
...
function main() {
  try {
    ...
    if (mode === 'record' && !credentialCheck.hasCredentials) {
      console.log(`${colors.yellow}⚠️  Recording mode requested but credentials missing. Tests will likely fail.${colors.reset}\n`);
      process.exit(1);
    }
