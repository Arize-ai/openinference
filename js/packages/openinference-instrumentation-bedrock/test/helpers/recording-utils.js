  ...
  switch (mode) {
    ...
    case "replay":
      if (fs.existsSync(recordingPath)) {
        ...
        console.warn(
          "   Tests may fail. Run with BEDROCK_RECORD_MODE=record to create recordings.",
        );
      }
      break;
    ...
    cleanup: () => {
      if (mode === "record") {
        ...
        if (recordings.length > 0) {
          ...
          fs.writeFileSync(
            recordingPath,
            JSON.stringify(sanitizedRecordings, null, 2),
          );
          console.log(
            `💾 Saved ${recordings.length} recordings to: ${recordingFile}`,
          );
        }
