...
export interface SpanTraceOptions<Fn extends AnyFn = AnyFn> {
   ...
   * the request to a different model, such as Anthropic's server-side
   * fallback, where a classifier-triggered refusal hands the request off to a
   * fallback model.
   *
   ...
   * request to a different model, such as Anthropic's server-side fallback,
   * where the response is generated by the fallback model rather than the
   * requested one.
   *
   * @see https://platform.claude.com/docs/en/build-with-claude/refusals-and-fallback#server-side-fallback
   *
