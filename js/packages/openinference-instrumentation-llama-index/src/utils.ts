...
function formatRetrievalDocuments(output: llamaindex.NodeWithScore<llamaindex.Metadata>[]) {
    ...
    output.forEach((document, index) => {
      ...
      if (node instanceof TextNode) {
        ...
        docs[`${prefix}.${SemanticConventions.DOCUMENT_METADATA}`] = JSON.stringify(nodeMetadata) ?? undefined;
      }
    });
    return docs;
}