...
public class TracedEmbeddingSpan extends TracedSpan {

    private TracedEmbeddingSpan(Span span, Scope scope, com.arize.instrumentation.TraceConfig config) {
        super(span, scope, config);
    }

    public static TracedEmbeddingSpan start(OITracer tracer, String name) {
        Span span = startSpan(tracer, name, SemanticConventions.OpenInferenceSpanKind.EMBEDDING);
        Scope scope = span.makeCurrent();
        return new TracedEmbeddingSpan(span, scope, tracer.getConfig());
    }
