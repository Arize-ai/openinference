 ...
 * 1. Start Phoenix: {@code docker run -p 6006:6006 -p 4317:4317 arizephoenix/phoenix:latest}
 * 2. (Optional) if you have auth enabled on Phoenix, {@code export PHOENIX_API_KEY=your-key-here}
 * 3. Run the example: {@code ./gradlew :examples:annotations-example:run}
 * 4. View traces in Phoenix: http://localhost:6006
 */
public class AnnotationsDemo {

    private static SdkTracerProvider tracerProvider;
    private static final Logger logger = Logger.getLogger(AnnotationsDemo.class.getName());
    ...
    public static void main(String[] args) {
        ...
        OITracer tracer = new OITracer(tracerProvider.get("annotations-example"));
        OpenInferenceAgent.register(tracer);

        logger.info("=== Annotation-Based Tracing Demo ===");

        // Use the annotated service — spans are created automatically by @TraceAgent, @TraceChain, etc.
    ...
    private static void initializeOpenTelemetry() {
        String projectName = System.getenv().getOrDefault("PROJECT_NAME", "java-annotations");

        Resource resource = Resource.getDefault()
                .merge(Resource.create(Attributes.of(
                        AttributeKey.stringKey("service.name"), "annotations-example",
        ...
        if (arizeApiKey != null && !arizeApiKey.isEmpty()) {
            ...
            otlpExporterBuilder = OtlpGrpcSpanExporter.builder()
                    ...
                    .setHeaders(() -> Map.of(
                            "api_key", arizeApiKey,
                            "space_id", arizeSpaceId != null ? arizeSpaceId : ""));
