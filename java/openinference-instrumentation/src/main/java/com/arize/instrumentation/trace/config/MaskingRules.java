...
class MaskingUtils {
    ...
    private static final List<MaskingRule> maskingRules = Arrays.asList(
            new MaskingRule(
                    args -> args.getConfig().isHideInputs() &&
                            args.getKey().equals(SemanticConventions.INPUT_VALUE),
                    args -> REDACTED_VALUE
            ),
            new MaskingRule(
                    args -> args.getConfig().isHideInputs() &&
            ...
            new MaskingRule(
                    args -> args.getConfig().isHideOutputs() &&
                            args.getKey().equals(SemanticConventions.OUTPUT_VALUE),
                    args -> REDACTED_VALUE
            ),
            new MaskingRule(
                    args -> args.getConfig().isHideOutputs() &&
            ...
            new MaskingRule(
                    args -> (args.getConfig().isHideOutputs() || args.getConfig().isHideOutputMessages()) &&
                            args.getKey().contains(SemanticConventions.LLM_OUTPUT_MESSAGES),
                    args -> null
            ),
            maskInputTextRule,
    ...
    public static Value mask(TraceConfig config, String key, Value value) {
        return mask(MaskingRuleArgs.builder()
                .config(config)
                .key(key)
                .value(value)
                .build());
