...
Use `output_messages` because this is still a chat completion message. Do not use span-root `output.audio.*` here. That prefix is for voice-session spans that have no message list. See [openai_realtime.md](./openai_realtime.md).
